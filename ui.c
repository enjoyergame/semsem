...
static void LoadBmpAsset(const char* path, HBITMAP* hbm, HDC* hdc_mem, HBITMAP* hbm_old, int* w, int* h, HDC screen_hdc) {
    *hbm = (HBITMAP)LoadImageA(NULL, path, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
    if (*hbm) {
        BITMAP bm;
        GetObject(*hbm, sizeof(BITMAP), &bm);
        *w = bm.bmWidth;
        *h = bm.bmHeight;
        *hdc_mem = CreateCompatibleDC(screen_hdc);
        *hbm_old = (HBITMAP)SelectObject(*hdc_mem, *hbm);
    } else {
        *w = 0; *h = 0; *hdc_mem = NULL;
    }
}

static void FreeBmpAsset(HBITMAP* hbm, HDC* hdc_mem, HBITMAP* hbm_old) {
    if (*hbm && *hdc_mem) {
...
bool UI_Init(void) {
    ...
    g_hwnd = CreateWindowEx(0, wc.lpszClassName, "Furry Porn", style, CW_USEDEFAULT, CW_USEDEFAULT, 800, 600, NULL, NULL, wc.hInstance, NULL);
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        LoadBmpAsset(p, &g_hbt[i], &g_hdt[i], &g_ot[i], &g_tw[i], &g_th[i], hdc);
    }

    // Загрузка оружия
    LoadBmpAsset("assets/weapon.bmp", &g_hbw, &g_hdw, &g_ow, &g_ww, &g_wh, hdc);
    ...
    for (int i = 0; i < 7; i++) {
        LoadBmpAsset(sf[i], &g_hbs[i], &g_hds[i], &g_os[i], &g_sw[i], &g_sh[i], hdc);
    }
    ...
    for (int i = 0; i < 4; i++) {
        LoadBmpAsset(ff[i], &g_hbmFaces[i], &g_hdcFaces[i], &g_hbmOldFaces[i], &g_faceWidth[i], &g_faceHeight[i], hdc);
    }
    ...
    if (state->mode == MODE_MENU) {
        ...
        for (int p = 0; p < num_views; p++) {
            ...
            if (g_hbw && !state->has_won) {
                int by = 0, bx = 0;
                // Улучшение 4: Используем переданный input вместо GetAsyncKeyState
                if (p == 0 && (input->w || input->s || input->a || input->d)) {
                    by = (int)(sin(GetTickCount() / 150.0f) * 15.0f);
                    bx = (int)(cos(GetTickCount() / 300.0f) * 10.0f);
                }
                if (p == 1 && (input->up || input->down || input->left || input->right)) {
                    by = (int)(sin(GetTickCount() / 150.0f) * 15.0f);
                    bx = (int)(cos(GetTickCount() / 300.0f) * 10.0f);
                }
...
void UI_Shutdown(void) {
    for (int i = 1; i <= 5; i++) FreeBmpAsset(&g_hbt[i], &g_hdt[i], &g_ot[i]);
    for (int i = 0; i < 7; i++) FreeBmpAsset(&g_hbs[i], &g_hds[i], &g_os[i]);
    for (int i = 0; i < 4; i++) FreeBmpAsset(&g_hbmFaces[i], &g_hdcFaces[i], &g_hbmOldFaces[i]);
    FreeBmpAsset(&g_hbw, &g_hdw, &g_ow);
